...
saveproperty!(file, address, p::Function) = @warn "Cannot save Function property into $address"

saveproperty!(file, address, p::Tuple) =
    [saveproperty!(file, address * "/$i", p[i]) for i in 1:length(p)]

saveproperty!(file, address, p) =
    [saveproperty!(file, address * "/$subp", getproperty(p, subp)) for subp in propertynames(p)]

# Special saveproperty! so boundary conditions are easily readable outside julia.
function saveproperty!(file, address, cbcs::CoordinateBoundaryConditions)
    for endpoint in propertynames(cbcs)
        ...
        if endpoint_bc.condition isa Function
            ...
            file[address * "/$endpoint/type"] = string(bctype(endpoint_bc))
            file[address * "/$endpoint/condition"] = missing
        else
            file[address * "/$endpoint/type"] = string(bctype(endpoint_bc))
            file[address * "/$endpoint/condition"] = endpoint_bc.condition
        end
    end
    return nothing
end
...
serializeproperty!(file, address, p::Function) = @warn "Cannot serialize Function property into $address"

function serializeproperty!(file, address, p::FieldBoundaryConditions)
    if has_reference(Function, p)
        @warn "Cannot serialize $address as it contains functions. Will replace with missing. " *
...
function serializeproperty!(file, address, p::Field{LX, LY, LZ}) where {LX, LY, LZ}
    serializeproperty!(file, address * "/location", (LX(), LY(), LZ()))
    serializeproperty!(file, address * "/data", p.data.parent)
    serializeproperty!(file, address * "/boundary_conditions", p.boundary_conditions)
    return nothing
end
...
function serializeproperty!(file, address, 
...
serializeproperty!(file, address, p::NamedTuple) =
    [serializeproperty!(file, address * "/$subp", getproperty(p, subp)) for subp in keys(p)]
